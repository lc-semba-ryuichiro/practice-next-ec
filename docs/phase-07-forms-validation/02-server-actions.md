...
- [Server Actions とは](#server-actions-とは)
- ['use server' ディレクティブ](#use-server-ディレクティブ)
  - [ファイル単位で定義](#ファイル単位で定義)
  - [関数単位で定義](#関数単位で定義)
- [フォームからの呼び出し](#フォームからの呼び出し)
  - [基本的な使い方](#基本的な使い方)
  - [Client Component からの呼び出し](#client-component-からの呼び出し)
- [データの受け取り](#データの受け取り)
  - [FormData から値を取得](#formdata-から値を取得)
  - [オブジェクトに変換](#オブジェクトに変換)
- [バリデーションとエラーレスポンス](#バリデーションとエラーレスポンス)
  - [戻り値を使ったエラーハンドリング](#戻り値を使ったエラーハンドリング)
- [セキュリティ考慮事項](#セキュリティ考慮事項)
  - [1. サーバーサイドでのバリデーション](#1-サーバーサイドでのバリデーション)
  - [2. 認証の確認](#2-認証の確認)
  - [3. CSRF 対策](#3-csrf-対策)
  - [4. 入力値のサニタイズ](#4-入力値のサニタイズ)
- [Server Actions vs API Routes](#server-actions-vs-api-routes)
  - [使い分けの基準](#使い分けの基準)
  - [Server Actions を使うべきケース](#server-actions-を使うべきケース)
  - [API Routes を使うべきケース](#api-routes-を使うべきケース)
- [実装パターン](#実装パターン)
  - [リダイレクトとリフレッシュ](#リダイレクトとリフレッシュ)
  - [プログレッシブエンハンスメント](#プログレッシブエンハンスメント)
- [EC サイトでの実装例](#ec-サイトでの実装例)
  - [商品レビュー投稿](#商品レビュー投稿)
  - [カートへの追加](#カートへの追加)
- [まとめ](#まとめ)
  - [Server Actions のメリット](#server-actions-のメリット)
  - [ベストプラクティス](#ベストプラクティス)
- [次のステップ](#次のステップ)
