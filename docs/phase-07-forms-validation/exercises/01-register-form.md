...
ZodとServer Actionsを使ってユーザー登録フォームを実装します。
...
- Phase 7の学習ドキュメント（01〜05）を読んでいること
...
- [ ] Zodスキーマが正しく動作する
- [ ] Server ActionがFormDataを正しく処理する
...
React 19以降が必要です。`package.json` を確認してください。
...
- Zodスキーマでバリデーションが動作する
- Server Actionがフォームデータを処理する
- エラーメッセージが適切に表示される
- 送信中状態が表示される
- 登録完了時にメッセージが表示される
