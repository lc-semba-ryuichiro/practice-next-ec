...
Next.js の App Router は、**ファイルベースルーティング**を採用しています。これは、`app/` ディレクトリ内のフォルダ構造がそのまま URL パスに対応するシステムです。従来の React アプリケーションのように、ルーティング設定ファイルを書く必要がありません。
...
`(groupName)` 形式でフォルダを作ると、URL を変えずにファイルを整理できます。
