...
2. **IP 制限**: 管理画面へのアクセスを特定の IP アドレスのみに制限してみましょう
...
- [ ] 一般ユーザーで /admin にアクセスすると権限エラーページが表示される
- [ ] 管理者で /admin にアクセスできる
- [ ] 静的ファイル（\_next/static, 画像など）は Middleware の影響を受けない
...
1. Middleware を使った認証チェックの実装
2. matcher によるルート指定
3. callbackUrl を使った UX 向上
4. ロールベースのアクセス制御
5. 型安全な認証情報の取得
...
お疲れさまでした。Phase 8 の演習をすべて完了しました。
