...
- [基本構文](#基本構文)
  - [テストの構造](#テストの構造)
  - [describe / it / test の違い](#describe--it--test-の違い)
- [アサーション（expect）](#アサーションexpect)
  - [基本的なマッチャー](#基本的なマッチャー)
  - [オブジェクトのマッチャー](#オブジェクトのマッチャー)
- [セットアップとティアダウン](#セットアップとティアダウン)
  - [beforeEach / afterEach](#beforeeach--aftereach)
  - [beforeAll / afterAll](#beforeall--afterall)
- [モック](#モック)
  - [vi.fn() - モック関数](#vifn---モック関数)
  - [vi.fn() の戻り値を設定](#vifn-の戻り値を設定)
  - [vi.mock() - モジュールモック](#vimock---モジュールモック)
  - [vi.spyOn() - 既存関数のスパイ](#vispyon---既存関数のスパイ)
- [EC サイトでの活用例](#ec-サイトでの活用例)
  - [カート計算ロジックのテスト](#カート計算ロジックのテスト)
- [NG / OK パターン](#ng--ok-パターン)
  - [NG: テストが独立していない](#ng-テストが独立していない)
  - [OK: 各テストが独立している](#ok-各テストが独立している)
  - [NG: 実装の詳細をテストしている](#ng-実装の詳細をテストしている)
  - [OK: 公開 API をテストしている](#ok-公開-api-をテストしている)
- [非同期テスト](#非同期テスト)
...
VitestはViteベースの高速なテストフレームワークです。
Jestとの互換性を持ちながら、ESMネイティブ対応やホットモジュールリプレースメント（HMR）による高速な開発体験を提供します。

このセクションでは、Vitestの基本的な使い方を学び、ECサイトのビジネスロジックをテストする方法を習得します。
...
- Viteのビルドパイプラインを利用し、Jestより高速に動作する
- `describe`, `it`, `expect` などのAPIがJestと互換性を持つ
- ES Modulesをネイティブで実行可能
- 設定不要でTypeScriptをサポート
- ファイル変更時に関連するテストのみ再実行するwatchモード
...
3. `beforeEach` と `beforeAll` の違いは何ですか？
   ...
   **回答例**: `beforeEach` は各テストの前に毎回実行される。`beforeAll` はdescribeブロック内のすべてのテストの前に1回だけ実行される。

4. `toBe` と `toEqual` の違いは何ですか？

   **回答例**: `toBe` は厳密など価（===）で比較する。プリミティブ値や同じ参照のオブジェクトに使用する。`toEqual` は深いなど価比較で、オブジェクトや配列の内容を再帰的に比較する。

***

## 次のステップ

Vitestの基礎を学んだら、次は [Testing Library](./02-testing-library.md) でコンポーネントテストを学びましょう。
