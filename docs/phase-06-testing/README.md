...
```mermaid
graph LR
    A["01 Vitest 基礎"] --> B["02 Testing Library"]
...
graph TD
    subgraph pyramid["テストピラミッド"]
        E2E["E2E テスト<br/>Playwright<br/>🔺 少量・高コスト・遅い"]
        INT["コンポーネントテスト<br/>Testing Library<br/>◆ 中量・中コスト"]
        UNIT["ユニットテスト<br/>Vitest<br/>◼ 大量・低コスト・高速"]
    end
...
graph TD
    ...
    subgraph component["コンポーネントテスト"]
        PC["ProductCard"]
        CI["CartItem"]
        FORM["フォーム"]
    end

    subgraph e2e["E2E テスト"]
        PURCHASE["購入フロー"]
        AUTH["認証フロー"]
        SEARCH["商品検索"]
    end
