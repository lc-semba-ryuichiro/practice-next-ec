...
- [useAtomValue](#useatomvalue)
  ...
  - [戻り値](#戻り値-1)
  - [使いどころ](#使いどころ-1)
  - [パフォーマンス上のメリット](#パフォーマンス上のメリット)
- [useSetAtom](#usesetatom)
  - [基本的な使い方](#基本的な使い方-2)
  - [戻り値](#戻り値-2)
  - [使いどころ](#使いどころ-2)
  - [パフォーマンス上のメリット](#パフォーマンス上のメリット-1)
- [実践例：EC サイトでの使い分け](#実践例ec-サイトでの使い分け)
  - [ヘッダーのカート表示](#ヘッダーのカート表示)
  - [商品カードの「カートに追加」ボタン](#商品カードのカートに追加ボタン)
  - [カートアイテムの数量変更](#カートアイテムの数量変更)
- [パフォーマンス最適化](#パフォーマンス最適化)
  - [問題：不要な再レンダリング](#問題不要な再レンダリング)
  - [最適化の例](#最適化の例)
- [使い分けのフローチャート](#使い分けのフローチャート)
  - [判断基準](#判断基準)
- [よくあるパターン](#よくあるパターン)
  - [パターン 1: 表示専用コンポーネント](#パターン-1-表示専用コンポーネント)
  - [パターン 2: アクション専用コンポーネント](#パターン-2-アクション専用コンポーネント)
  - [パターン 3: 入力フォーム](#パターン-3-入力フォーム)
- [アンチパターン](#アンチパターン)
  - [1. すべてに useAtom を使う](#1-すべてに-useatom-を使う)
  - [2. 読み取り専用 Atom に useAtom を使う](#2-読み取り専用-atom-に-useatom-を使う)
  - [3. コンポーネントの責務が不明確](#3-コンポーネントの責務が不明確)
- [まとめ](#まとめ)
  - [フックの選択基準](#フックの選択基準)
  - [パフォーマンスのポイント](#パフォーマンスのポイント)
- [次のステップ](#次のステップ)

## 概要

Jotaiには3つの主要なフックがあります。
...
`useSetAtom` はAtomの値が変わっても**再レンダリングされません**。
