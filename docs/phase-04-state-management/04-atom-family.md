...
- [atomFamily とは](#atomfamily-とは)
  - [使いどころ](#使いどころ)
- [基本的な使い方](#基本的な使い方)
  - [インポート](#インポート)
  - [基本構文](#基本構文)
- [EC サイトでの実装例](#ec-サイトでの実装例)
  - [商品ごとの数量管理](#商品ごとの数量管理)
  - [コンポーネントでの使用](#コンポーネントでの使用)
  - [商品カードでの使用](#商品カードでの使用)
- [パラメータのなど価性](#パラメータのなど価性)
  - [デフォルトのなど価性チェック](#デフォルトのなど価性チェック)
  - [カスタムなど価性関数](#カスタムなど価性関数)
  - [安定した参照を使用](#安定した参照を使用)
- [メモ化とキャッシュ](#メモ化とキャッシュ)
  - [Atom のキャッシュ](#atom-のキャッシュ)
  - [キャッシュのクリア](#キャッシュのクリア)
- [派生 Atom との組み合わせ](#派生-atom-との組み合わせ)
  - [商品ごとの小計を計算](#商品ごとの小計を計算)
  - [カート全体の合計](#カート全体の合計)
- [実践例：お気に入りボタン](#実践例お気に入りボタン)
  - [商品ごとのお気に入り状態](#商品ごとのお気に入り状態)
  - [お気に入りボタンコンポーネント](#お気に入りボタンコンポーネント)
- [ベストプラクティス](#ベストプラクティス)
  - [1. プリミティブなパラメータを使う](#1-プリミティブなパラメータを使う)
  - [2. 命名規則を統一する](#2-命名規則を統一する)
  - [3. 不要になった Atom を削除する](#3-不要になった-atom-を削除する)
- [よくある間違い](#よくある間違い)
  - [1. 毎レンダリングで新しい Atom を作成](#1-毎レンダリングで新しい-atom-を作成)
  - [2. など価性チェックを忘れる](#2-など価性チェックを忘れる)
- [まとめ](#まとめ)
  - [atomFamily のポイント](#atomfamily-のポイント)
  - [使いどころ](#使いどころ-1)
- [次のステップ](#次のステップ)

## atomFamily とは

**atomFamily** は、パラメータに応じてAtomを動的に生成する関数です。
同じ構造のAtomを複数作成する必要がある場合に便利です。
...
atomFamilyはパラメータのなど価性を厳密に（`===`）比較します。
...
オブジェクトをパラメータにする場合は、カスタムなど価性関数を使用します。
...
atomFamilyは作成したAtomをキャッシュします。
同じパラメータで呼び出すと、同じAtomインスタンスが返されます。
...
- パラメータに応じてAtomを動的に生成
- 同じパラメータには同じAtomを返す（キャッシュ）
- オブジェクトパラメータにはなど価性関数が必要
- 不要になったAtomは削除可能
...
atomFamilyを理解したら、[フックの使い分け](./05-hooks-comparison.md) に進んで、useAtom / useAtomValue / useSetAtomの最適な使い分けを学びましょう。
