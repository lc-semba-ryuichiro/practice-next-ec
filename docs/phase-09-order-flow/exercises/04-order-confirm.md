...
- [完成イメージ](#完成イメージ)
- [前提条件](#前提条件)
- [ステップ 1: ページの作成](#ステップ-1-ページの作成)
- [ステップ 2: 注文サマリー](#ステップ-2-注文サマリー)
- [ステップ 3: 確認フォーム（二重送信防止）](#ステップ-3-確認フォーム二重送信防止)
- [ステップ 4: 注文処理 Server Action](#ステップ-4-注文処理-server-action)
- [ステップ 5: 二重送信防止の強化](#ステップ-5-二重送信防止の強化)
  - [5.1 トークンを使用した対策](#51-トークンを使用した対策)
  - [5.2 ページでトークンを生成](#52-ページでトークンを生成)
  - [5.3 Server Action でトークンを検証](#53-server-action-でトークンを検証)
- [確認ポイント](#確認ポイント)
- [よくある問題](#よくある問題)
  - [1. リダイレクトが動作しない](#1-リダイレクトが動作しない)
  - [2. チェックアウトデータが見つからない](#2-チェックアウトデータが見つからない)
- [次の演習](#次の演習)

## 目標

購入フローの4番目のステップとして、注文内容を最終確認し、注文を確定する画面を実装します。
...
Server Actions内で `redirect()` を呼び出す際、try-catchで囲むと正常に動作しない場合があります。redirectは例外をスローするため、catchで捕捉されてしまいます。
...
Cookieの有効期限が切れている可能性があります。「セッションが期限切れです。カートからやり直してください。」のようなエラーメッセージを表示し、カート画面へのリンクを提示してください。
